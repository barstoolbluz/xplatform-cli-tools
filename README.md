...
All tools are declaratively defined in TOML in the environment's manifest ([see About Flox](#about-flox)) and are automatically installed when you activate the environment—no manual installation needed. The environment wraps these tools with secure credential handling, and you can easily extend this pattern to other CLI tools ([see Extensibility](#extensibility)).
